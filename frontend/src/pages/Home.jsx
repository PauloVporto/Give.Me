...
export default function Home() {
  ...
  const toggleFavorite = useCallback(async (e, itemId) => {
    ...
    const token = localStorage.getItem(ACCESS_TOKEN);
    if (!token) {
      setAlertState({ open: true, message: "Você precisa fazer login para favoritar itens", severity: "warning" });
      return;
    }
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        setAlertState({ open: true, message: "Erro ao favoritar item", severity: "error" });
      }
